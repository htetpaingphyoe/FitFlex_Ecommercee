...
// Fetch users based on search term (if any)
if (isset($_GET['term']) && !empty($_GET['term'])) {
  $filtervalue = $_GET['term'];

  // Check if the search term is numeric (for User ID)
  if (is_numeric($filtervalue)) {
    // Exact match for User ID
    $query = "SELECT * FROM users WHERE User_Id = $filtervalue LIMIT $limit OFFSET $offset";
  } else {
    // Fallback to LIKE for other fields (e.g., First_name, Last_name, etc.)
    $query = "SELECT * FROM users WHERE CONCAT(First_name, ' ', Last_name, ' ', Gmail) LIKE '%$filtervalue%' LIMIT $limit OFFSET $offset";
  }
} else {
      ...
      <section class="main-content w-full p-6">
        ...
        <div class="flex justify-end mb-2">
          <form method="GET" action="" class="flex border-2 border-gray-500 overflow-hidden max-w-md mx-auto font-[sans-serif] rounded-lg">
            <input type="text" name="term" id="searchInput" placeholder="Search user by name or ID..." class="w-full lg:w-[400px] outline-none bg-white text-gray-600 text-sm px-4 py-3" value="<?= isset($_GET['term']) ? htmlspecialchars($_GET['term']) : '' ?>" />
        ...
        <div class="overflow-x-auto rounded-lg">
          <table class="min-w-full border-2 border-black">
            <thead class="bg-gray-800 whitespace-nowrap border-2 border-black">
              <tr>
                <th class="p-4 text-left text-sm font-medium text-white text-center">ID</th>
                <th class="p-2 text-left text-sm font-medium text-white text-center">First Name</th>
                <th class="p-2 text-left text-sm font-medium text-white text-center">Last Name</th>
                <th class="p-2 text-left text-sm font-medium text-white">Address</th>
                <th class="p-2 text-left text-sm font-medium text-white text-center">Phone Number</th>
                <th class="p-2 text-left text-sm font-medium text-white">Gmail</th>
                <th class="p-2 text-left text-sm font-medium text-white text-center">Password</th>
                <th class="p-4 text-left text-sm font-medium text-white text-center">Actions</th>
              </tr>
            </thead>
            <tbody id="tableBody" class="whitespace-nowrap bg-gray-200">
              ...
              ?>
                  <tr class="even:bg-blue-50">
                    <td class="p-2 text-center text-sm text-black"><?= $user["User_Id"] ?></td>
                    <td class="p-2 text-sm text-black"><?= $user["First_name"] ?></td>
                    <td class="p-2 text-sm text-black"><?= $user["Last_name"] ?></td>
                    <td class="p-2 text-sm text-black"><?= $user["Address"] ?></td>
                    <td class="p-2 text-sm text-black"><?= $user["Ph_number"] ?></td>
                    <td class="p-2 text-sm text-black"><?= $user["Gmail"] ?></td>
                    <td class="p-2 text-sm text-center text-black"><?= $user["Password"] ?></td>
                    <td class="p-2">
                      <div class="flex item-center gap-2">
                        <a href="useredit.php?id=<?= $user["User_Id"] ?>" class="font-medium text-blue-600 dark:text-gray-100 item-center bg-green-400 h-full p-4 rounded-md hover:bg-yellow-500">
                ...
                ?>
                <tr>
                  <td colspan="8" class="p-4 text-center text-gray-600">No record found!</td>
                </tr>
              <?php
              }
              ?>
